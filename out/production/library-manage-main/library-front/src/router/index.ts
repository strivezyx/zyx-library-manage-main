...
router.beforeEach((to) => {
  ...
  if (to.path.startsWith('/reader-client')) {
    if (store.loginRole === 'reader' && store.readerInfo?.token) return true
    return '/login'
  }

  if (store.loginRole === 'manager' && store.managerInfo?.token) {
    return true
  }

  if (store.loginRole === 'reader' && store.readerInfo?.token) {
